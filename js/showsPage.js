...
$("#subs-tbl").on("click", "tr", function() {
	$("#subs-tbl").DataTable().rows().every(function(){
...
function runEditModal(){
	var showData = $('#subs-tbl').DataTable().row('.selected').data();
	var showName = showData.showName;
	console.log('Editing: ' + showName);
	db.collection('shows').doc(showName).get()
		.then(function(doc){
			if (doc.exists){
				...
				$("#subs-tbl thead tr th").each(function(){
					...
					var index = subs.indexOf(columnName);
...
function initializeSubsTable(){
	...
	var tableHeader = document.getElementById("headers");
	var columnsList = [{data:"showName", title:"Show Name"}];
	...
	db.collection("users").get().then(function(querySnapshot) {
		...
		var table = $('#subs-tbl').DataTable({	iDisplayLength: 15,
			order: [[0, "asc"]],
			columns: columnsList,
			lengthChange: false
		});
		...
		$('#subs-tbl').DataTable({	iDisplayLength: 15,
			order: [[0, "asc"]],
			columns: columnsList,
			lengthChange: false
		});
...
function populateSubsTable(table) {
	...
	db.collection('shows').get().then(function(querySnapshot){
		querySnapshot.forEach(function(showDoc){
			var showName = showDoc.id;
			console.log("Adding show to table: " + showName);
			var show = showDoc.data();
			var row = {showName: showDoc.id};

			for (i in show.subs){
				var subName = show.subs[i];
				row[subName] = "yes";
			}
			table.row.add(row);
		});
...
function doesShowExist(showName){
	...
	var rowData = $('#subs-tbl').DataTable().rows().data();
